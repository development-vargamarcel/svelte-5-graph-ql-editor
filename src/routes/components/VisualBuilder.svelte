<script>
    ...
    const unsubscribe = graphqlStore.subscribe(state => {
      ...
      if (state.queryStructure && state.queryStructure.operations.length > 0) {
        ...
        if (activeOp) {
          ...
          const currentOpString = JSON.stringify(currentOperation);
          const newOpString = JSON.stringify(activeOp);
          
          if (currentOpString !== newOpString) {
            console.log('[v0] VisualBuilder: Syncing visual builder with parsed query structure');
            currentOperation = JSON.parse(JSON.stringify(activeOp)); // Deep clone
          }
        }
      } else if (!state.queryStructure || state.queryStructure.operations.length === 0) {
        console.log('[v0] VisualBuilder: No operations found, resetting to empty state');
  ...
  function addField(parentFields = currentOperation?.fields || [], typeName = 'Query', fieldPath = []) {
    ...
    if (!currentOperation) {
      console.log('[v0] VisualBuilder: Creating new operation since none exists');
      currentOperation = {
        type: 'query',
...
<div class="h-full overflow-y-auto transition-colors duration-200 {darkMode ? 'text-white' : 'text-gray-900'}">
  <div class="space-y-6">
    ...
    {:else}
      <!-- Enhanced empty state when no operation exists -->
      <div class="p-8 rounded-lg border text-center transition-colors duration-200 {darkMode ? 'bg-gray-800 border-gray-700 text-gray-400' : 'bg-white border-gray-200 text-gray-500'}">
